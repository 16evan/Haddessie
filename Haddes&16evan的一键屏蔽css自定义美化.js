...
// @version      2.0
// @description  智能区分系统CSS和创作者自定义CSS，一键屏蔽高耗能特效（动画/背景/阴影），保留网页基础功能布局。
...
(function() {
    ...
    const performanceCSS = `
        ...
        *:not([class*="translate-x-[-50%]"]) {
            /* 注意：这里稍微放宽了一点，防止误伤定位元素，但在下面专门处理动画 */
            /* 如果不是居中定位元素，强制移除 transform */
            /* 但为了更稳妥，我们主要针对动画和阴影 */
        }
        ...
        * {
        ...
        /* 2. 针对普通元素，移除 transform 以节省性能 */
        /* 但排除掉那些带有 translate-x-[-50%] 的元素（即弹窗） */
        *:not([class*="translate-x-[-50%]"]):not([class*="translate-y-[-50%]"]) {
            transform: none !important;
        }

        /* 3. 【核心修复】强制修正弹窗位置 */
        /* 只要元素包含这两个类名，就强制应用居中位移，覆盖上面的 none */
